...
while true; do
    ...
    if [ -f "${LOCAL_PROJECT_DIR}${LOCAL_VENV_NAME}/bin/pip" ]; then
        ...
        if [ -f "${LOCAL_PROJECT_DIR}sync/requirements-common.txt" ]; then
            EXISTING_PACKAGES=$(grep -v '^#' "${LOCAL_PROJECT_DIR}sync/requirements-common.txt" | sort)
        else
            EXISTING_PACKAGES=""
        fi
        
        if [ "$CURRENT_PACKAGES" != "$EXISTING_PACKAGES" ]; then
            pip freeze > "${LOCAL_PROJECT_DIR}sync/requirements-common.txt.tmp"
            # Add header comment to common requirements
            echo "# Common requirements (Mac + Pi)" > "${LOCAL_PROJECT_DIR}sync/requirements-common.txt"
            cat "${LOCAL_PROJECT_DIR}sync/requirements-common.txt.tmp" >> "${LOCAL_PROJECT_DIR}sync/requirements-common.txt"
            rm "${LOCAL_PROJECT_DIR}sync/requirements-common.txt.tmp"
    ...
    if [ $? -eq 0 ]; then
        ...
        COMBINED_REQ_HASH=""
        if [ -f "${LOCAL_PROJECT_DIR}sync/requirements-common.txt" ] && [ -f "${LOCAL_PROJECT_DIR}sync/requirements-pi.txt" ]; then
            # Cross-platform checksum (macOS uses md5, Linux uses md5sum)
            if command -v md5sum >/dev/null 2>&1; then
                COMMON_HASH=$(md5sum "${LOCAL_PROJECT_DIR}sync/requirements-common.txt" 2>/dev/null | cut -d' ' -f1 || echo "")
                PI_HASH=$(md5sum "${LOCAL_PROJECT_DIR}sync/requirements-pi.txt" 2>/dev/null | cut -d' ' -f1 || echo "")
            elif command -v md5 >/dev/null 2>&1; then
                COMMON_HASH=$(md5 -q "${LOCAL_PROJECT_DIR}sync/requirements-common.txt" 2>/dev/null || echo "")
                PI_HASH=$(md5 -q "${LOCAL_PROJECT_DIR}sync/requirements-pi.txt" 2>/dev/null || echo "")
            else
        ...
        if [ "$SKIP_DEPS" = false ]; then
            ...
            ssh "$REMOTE_HOST" "cd $REMOTE_HOST_PATH && \
                ...
                grep -v '^#' sync/requirements-common.txt >> requirements.txt && \
                echo '' >> requirements.txt && \
                echo '# Pi-specific packages' >> requirements.txt && \
                grep -v '^#' sync/requirements-pi.txt >> requirements.txt && \
